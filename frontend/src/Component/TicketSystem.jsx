...
const TicketSystem = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name:'', email:'', priority:'medium', department:'general', subject:'', description:'' });
      setSelectedFiles([]);
      onSuccess?.();
    } catch (error) {
  ...
  return (
    <div>
        ...
        <div className={`p-4 rounded-2xl mb-6 ${submitStatus.type === 'success'
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              {ticketNumber && <p className="text-sm mt-1">Your ticket number: <span className="font-bold">{ticketNumber}</span></p>}
            </div>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-green-700 mb-1">Priority *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-green-700 mb-1">Attachments (Optional)</label>
